...
The ircoder server has moved to the Effect HttpApi backend. Remaining work is
...
- plugins get global "ircoder" instance like in that post i showed
- ircoder instance has stuff like `ircoder.session.prompt()` or
  `ircoder.tool.register({...})`
