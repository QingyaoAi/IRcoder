...
brew install anomalyco/tap/ircoder # macOS 和 Linux（推荐，始终保持最新）
brew install ircoder              # macOS 和 Linux（官方 brew formula，更新频率较低）
sudo pacman -S ircoder            # Arch Linux (Stable)
paru -S ircoder-bin               # Arch Linux (Latest from AUR)
mise use -g ircoder               # 任意系统
nix run nixpkgs#ircoder           # 或用 github:anomalyco/ircoder 获取最新 dev 分支
```
...
IRcoder 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/anomalyco/ircoder/releases) 或 [ircoder.ai/download](https://ircoder.ai/download) 下载。
...
1. `$IRCODER_INSTALL_DIR` - 自定义安装目录
2. `$XDG_BIN_DIR` - 符合 XDG 基础目录规范的路径
3. `$HOME/bin` - 如果存在或可创建的用户二进制目录
4. `$HOME/.ircoder/bin` - 默认备用路径
...
IRcoder 内置两种 Agent，可用 `Tab` 键快速切换：
...
了解更多 [Agents](https://ircoder.ai/docs/agents) 相关信息。

### 文档

更多配置说明请查看我们的 [**官方文档**](https://ircoder.ai/docs)。
...
如果你在项目名中使用了 “ircoder”（如 “ircoder-dashboard” 或 “ircoder-mobile”），请在 README 里注明该项目不是 IRcoder 团队官方开发，且不存在隶属关系。

---

**加入我们的社区** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=738j8655-cd59-4633-a30a-1124e0096789&qr_code=true) | [X.com](https://x.com/ircoder)
