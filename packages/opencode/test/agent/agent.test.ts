...
it.instance("plan agent denies edits except .ircoder/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".ircoder/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".ircoder", "skill", "perm-skill")
...
`,
      ...
      process.env.IRCODER_TEST_HOME = test.directory
