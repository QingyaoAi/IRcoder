...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("ircoder-find")
  api.delete("ircoder-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("ircoder-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("ircoder-find", new Highlight(...rest))
    return true
  }
