...
import { createIrcoder, type ToolPart } from "@ircoder/sdk"
...
console.log("🚀 Starting ircoder server...")
const ircoder = await createIrcoder({
  port: 0,
})
console.log("✅ Ircoder server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await ircoder.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new ircoder session...")
    const { client, server } = ircoder
    ...
    console.log("✅ Created ircoder session:", createResult.data.id)
  ...
  console.log("📝 Sending to ircoder:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Ircoder response:", JSON.stringify(result, null, 2))
