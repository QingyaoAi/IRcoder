...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.ircoder.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `ircoder.settings.dat` is special: it maps to the `ircoder.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"ircoder.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // ircoder.settings.dat → the electron settings store ("ircoder.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "ircoder.global.dat").
  const storeName = filename === "ircoder.settings.dat" ? "ircoder.settings" : filename
