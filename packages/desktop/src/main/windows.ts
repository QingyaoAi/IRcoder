...
import { resolveThemeVariant } from "@ircoder/ui/theme/resolve"
import type { DesktopTheme } from "@ircoder/ui/theme/types"
...
function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    ...
    void show(
      "IRcoder window terminated unexpectedly",
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show("IRcoder is not responding", "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
