...
import "./styles.css"
import { useTheme } from "@ircoder/ui/theme"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.IRCODER_CHANNEL === "prod" &&
...
const deepLinkEvent = "ircoder:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__IRCODER__ ??= {}
  const pending = window.__IRCODER__.deepLinks ?? []
  window.__IRCODER__.deepLinks = [...pending, ...urls]
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("ircoder.global.dat").getItem("language")
