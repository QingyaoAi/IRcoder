...
const channel = (() => {
  ...
  if (process.env.IRCODER_CHANNEL === "latest") return "prod"
  return "dev"
})()
...
export default defineConfig({
  main: {
    define: {
      "import.meta.env.IRCODER_CHANNEL": JSON.stringify(channel),
    },
    build: {
      rollupOptions: {
      ...
      },
      {
        ...
        async writeBundle() {
          for (const l of await fs.readdir(IRCODER_SERVER_DIST)) {
            ...
            await fs.writeFile(`./out/main/chunks/${l}`, await fs.readFile(`${IRCODER_SERVER_DIST}/${l}`))
          }
        },
      },
    ],
