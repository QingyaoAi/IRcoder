...
- Author package entrypoint is `@ircoder-ai/plugin/tui`.
...
```json
{
  "$schema": "https://ircoder.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/ircoder-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@ircoder-ai/plugin/tui`.
- `@ircoder-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@ircoder-ai/plugin/tui"
...
- The value is a semver range checked against the running IRcoder version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.ircoder` is absent, no check is performed (backward compatible).
...
- `ircoder plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `ircoder plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.ircoder` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.ircoder`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.ircoder`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.ircoder` dir or `IRCODER_CONFIG_DIR`, IRcoder installs `@ircoder-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@ircoder-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus IRcoder's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
IRcoder registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, IRcoder pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@ircoder-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"ircoder"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `ircoder.default`.
...
- Local plugins persist installed themes under the local `.ircoder/themes` area near the plugin config source.
...
- `--pure` / `IRCODER_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.ircoder/plugins/tui-smoke.tsx`
- Local vim plugin: `.ircoder/plugins/tui-vim.tsx`
- Local smoke config: `.ircoder/tui.json`
- Local smoke theme: `.ircoder/plugins/smoke-theme.json`
