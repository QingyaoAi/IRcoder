...
Delete `packages/ircoder/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/ircoder/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/ircoder/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/ircoder`
- TUI plugin loader tests in `packages/ircoder` if runtime plugin API wiring changed
