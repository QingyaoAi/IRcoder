...
Practical reference for a future `packages/server` split after the ircoder
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/ircoder`.
...
Until enough shared service code lives outside `packages/ircoder`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/ircoder`

It should not import `packages/ircoder` services while `packages/ircoder`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on ircoder-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/ircoder` implementation details.
