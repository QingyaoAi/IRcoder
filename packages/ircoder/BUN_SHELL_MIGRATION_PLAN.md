...
Replace runtime Bun shell template-tag usage in `packages/ircoder/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/ircoder/src` is removed except:
