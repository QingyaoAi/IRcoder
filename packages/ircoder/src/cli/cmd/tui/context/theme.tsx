...
import ircoder from "./theme/ircoder.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@ircoder/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "ircoder")
        draft.active = typeof active === "string" ? active : "ircoder"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.ircoder, store.mode)
    })
