...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for ircoder.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal ircoder sessions
...
ircoder acp --cwd /path/to/project
```
...
```bash
IRCODER_ENABLE_QUESTION_TOOL=1 ircoder acp
```
...
- **Terminal Integration**: Full terminal support via ircoder's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | ircoder acp
```
...
ACP sessions map cleanly to ircoder's internal session model:
