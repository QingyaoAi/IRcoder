...
const MANAGED_PLIST_DOMAIN = "ai.ircoder.managed"

// Keys injected by macOS/MDM into the managed plist that are not IRcoder config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/ircoder"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "ircoder")
    default:
      return "/etc/ircoder"
  }
}
...
export function managedConfigDir() {
  return process.env.IRCODER_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
