...
import { IRcoderHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for ircoder serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(IRcoderHttpApi)
...
export const PublicApi = IRcoderHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "ircoder",
    version: "1.0.0",
    description: "ircoder api",
