...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "ircoder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "ircoder.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ircoder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ircoder.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["ircoder.json", "ircoder.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "ircoder.json"),
        path.join(tmp.path, "ircoder.jsonc"),
        path.join(project, "ircoder.json"),
        path.join(project, "ircoder.jsonc"),
      ])
    })
  })
