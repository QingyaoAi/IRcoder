...
import { EffectFlock } from "@ircoder/core/util/effect-flock"
...
function remoteConfigClient(input: {
  ...
  return HttpClient.make((request) => {
    if (request.url.includes(".well-known/ircoder")) {
...
const schemaConfig = (config: object) => ({ $schema: "https://ircoder.ai/config.json", ...config })
...
const managedConfigDir = process.env.IRCODER_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.IRCODER_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.IRCODER_CONSOLE_TOKEN
  else process.env.IRCODER_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})

const writeManagedSettingsEffect = (settings: object, filename?: string) =>
  AppFileSystem.use.writeWithDirs(path.join(managedConfigDir, filename ?? "ircoder.json"), JSON.stringify(settings))

async function writeConfig(dir: string, config: object, name = "ircoder.json") {
  await Filesystem.write(path.join(dir, name), JSON.stringify(config))
}

const writeConfigEffect = (dir: string, config: object, name = "ircoder.json") =>
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.all(
      [
        ...
        input.local ? writeConfigEffect(path.join(directory, ".ircoder"), schemaConfig(input.local)) : undefined,
      ].filter(
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://ircoder.ai/config.json",
      snapshot: false,
    })
...
it.effect("creates global jsonc config with schema when no global configs exist", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const content = yield* AppFileSystem.use.readFileString(path.join(dir, "ircoder.jsonc"))
      expect(content).toContain('"$schema": "https://ircoder.ai/config.json"')
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
  ),
)

it.effect("does not create global config when IRCODER_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
    ...
    yield* withGlobalConfig({}, ({ dir }) =>
      withProcessEnv(
        ...
        Effect.gen(function* () {
          ...
          expect(yield* AppFileSystem.use.existsSafe(path.join(dir, "ircoder.jsonc"))).toBe(false)
        }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
      ),
    )
  }),
...
it.instance("updates config and preserves empty shell sentinel", () =>
  Effect.gen(function* () {
    ...
    yield* writeConfigEffect(
      test.directory,
      { $schema: "https://ircoder.ai/config.json", shell: "bash" },
      "config.json",
    )
...
it.effect("updates global config and omits empty shell key in json", () =>
  withGlobalConfig({ config: { shell: "bash" } }, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const writtenConfig = yield* AppFileSystem.use.readJson(path.join(dir, "ircoder.json"))
...
it.effect("updates global config and omits empty shell key in jsonc", () =>
  withGlobalConfig({ config: { shell: "bash", model: "test/model" }, name: "ircoder.jsonc" }, ({ dir }) =>
    Effect.gen(function* () {
      yield* Config.use.updateGlobal({ shell: "" })

      const file = path.join(dir, "ircoder.jsonc")
...
it.instance("ignores legacy tui keys in ircoder config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
...
it.instance("loads JSONC config file", () =>
  Effect.gen(function* () {
    ...
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, "ircoder.jsonc"),
      `{
        // This is a comment
        "$schema": "https://ircoder.ai/config.json",
        "model": "test/model",
...
it.instance("preserves env variables when adding $schema to config", () =>
  withProcessEnv(
    ...
    Effect.gen(function* () {
      ...
      yield* AppFileSystem.use.writeWithDirs(
        path.join(test.directory, "ircoder.json"),
      ...
      const content = yield* AppFileSystem.use.readFileString(path.join(test.directory, "ircoder.json"))
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { ircoder: { options: { apiKey: "{env:IRCODER_CONSOLE_TOKEN}" } } },
        }),
      ),
...
accountTokenIt.instance("resolves env templates in account config with account token", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(config.provider?.["ircoder"]?.options?.apiKey).toBe("st_test_token")
  }),
)
...
it.instance("throws error for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(path.join(test.directory, "ircoder.json"), "{ invalid json }")
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
it.instance("loads config from .ircoder directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".ircoder", "agent", "test.md"),
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".ircoder", "agent", "ordered.md"),
      `---
permission:
  bash: allow
  "*": deny
...
it.instance("loads agents from .ircoder/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".ircoder", "agents", "helper.md"),
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".ircoder", "agents", "nested", "child.md"),
      `---
model: test/model
mode: subagent
---
...
it.instance("loads commands from .ircoder/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".ircoder", "command", "hello.md"),
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".ircoder", "command", "nested", "child.md"),
      `---
...
it.instance("loads commands from .ircoder/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".ircoder", "commands", "hello.md"),
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".ircoder", "commands", "nested", "child.md"),
      `---
description: Nested command
---
Nested command template`,
...
it.effect("does not try to install dependencies in read-only IRCODER_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("IRCODER_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
)

it.effect("installs dependencies in writable IRCODER_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.effect("global config remains global when project config is disabled", () =>
  withConfigTree(
    ...
    withProcessEnv(
      "IRCODER_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const config = yield* Config.use.get()
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".ircoder", "agent", "helper.md"),
      `---
model: test/model
mode: subagent
---
...
// Note: preload.ts sets IRCODER_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
it.instance("managed jsonc settings override managed json settings", () =>
  Effect.gen(function* () {
    ...
    yield* writeManagedSettingsEffect({ model: "managed/jsonc" }, "ircoder.jsonc")
...
it.instance("local .ircoder config can override MCP from project config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://ircoder.ai/config.json",
    ...
    yield* AppFileSystem.use.ensureDir(path.join(test.directory, ".ircoder"))
    yield* writeConfigEffect(
      path.join(test.directory, ".ircoder"),
      {
        $schema: "https://ircoder.ai/config.json",
        mcp: {
          docs: {
...
remoteProjectOverride.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const config = yield* Config.use.get()
      expect(remoteProjectOverride.seen.wellKnown).toBe("https://example.com/.well-known/ircoder")
...
trailingSlashWellKnown.it.instance("wellknown URL with trailing slash is normalized", () =>
  Effect.gen(function* () {
    ...
    expect(trailingSlashWellKnown.seen.wellKnown).toBe("https://example.com/.well-known/ircoder")
  }),
)
      ...
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            ...
            expect(fetchedUrl).toBe(`${server.url.origin}/.well-known/ircoder`)
            expect(config.mcp?.jira?.enabled).toBe(true)
          }),
        ),
      { git: true },
...
const templatedHeaderWellKnown = wellKnown({
  remoteConfig: {
    url: "https://config.example.com/ircoder.json",
...
templatedHeaderWellKnown.it.instance("wellknown remote_config supports templated env vars in headers", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(templatedHeaderWellKnown.seen.wellKnown).toBe("https://example.com/.well-known/ircoder")
    expect(templatedHeaderWellKnown.seen.remote).toBe("https://config.example.com/ircoder.json")
...
const remotePrecedenceWellKnown = wellKnown({
  ...
  remoteConfig: { url: "https://config.example.com/{env:TEST_TOKEN}/ircoder.json" },
  remote: {
...
remotePrecedenceWellKnown.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const config = yield* Config.use.get()
      expect(remotePrecedenceWellKnown.seen.remote).toBe("https://config.example.com/test-token/ircoder.json")
      expect(config.mcp?.confluence?.enabled).toBe(true)
    }),
)
...
const nullConfigWellKnown = wellKnown({
  wellKnown: {
    config: null,
    remote_config: { url: "https://config.example.com/ircoder.json" },
  },
  remote: {
    mcp: { confluence: { type: "remote", url: "https://confluence.example.com/mcp", enabled: true } },
  },
...
nullConfigWellKnown.it.instance("wellknown config null is treated as absent", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(nullConfigWellKnown.seen.remote).toBe("https://config.example.com/ircoder.json")
    expect(config.mcp?.confluence?.enabled).toBe(true)
  }),
)

const invalidRemoteWellKnown = wellKnown({
  remoteConfig: { url: "https://config.example.com/ircoder.json" },
...
invalidRemoteWellKnown.it.instance("wellknown remote_config rejects non-object config responses", () =>
  Effect.gen(function* () {
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(invalidRemoteWellKnown.seen.remote).toBe("https://config.example.com/ircoder.json")
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    const file = path.join(tmp.path, "ircoder.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-ircoder@2.4.3", file)).toBe("oh-my-ircoder@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-ircoder@2.4.3", "file:///project/.ircoder/plugin/oh-my-ircoder.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.ircoder/plugin/demo.ts", "file:///project/.ircoder/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.ircoder/plugin/demo.ts"])
  })
  ...
  it.effect("loads auto-discovered local plugins as file urls", () =>
    withConfigTree(
      ...
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* AppFileSystem.use.writeWithDirs(
          path.join(test.directory, ".ircoder", "plugin", "my-plugin.js"),
          "export default {}",
        )
...
describe("IRCODER_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
  ...
  it.instance("skips project .ircoder/ directories when flag is set", () =>
    withProcessEnv(
      "IRCODER_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* AppFileSystem.use.writeWithDirs(
          path.join(test.directory, ".ircoder", "command", "test-cmd.md"),
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { IRCODER_CONFIG_DIR: undefined, IRCODER_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "IRCODER_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { IRCODER_DISABLE_PROJECT_CONFIG: "true", IRCODER_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed IRCODER_PERMISSION JSON used to crash
...
describe("IRCODER_PERMISSION env var", () => {
  it.instance("does not crash when IRCODER_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "IRCODER_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("IRCODER_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in IRCODER_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
        "IRCODER_CONFIG_CONTENT",
        JSON.stringify({
          $schema: "https://ircoder.ai/config.json",
  ...
  it.instance("substitutes {file:} tokens in IRCODER_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "IRcoder Managed",
          PayloadIdentifier: "ai.ircoder.managed.test",
          PayloadType: "ai.ircoder.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://ircoder.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://ircoder.ai/config.json")
})
