// Subprocess test harness for the ircoder CLI. Spawns the real binary against
...
//
// Configuration flows through ircoder's built-in test affordances:
//   - IRCODER_CONFIG_CONTENT      : provider config inline, no files to find
//   - IRCODER_TEST_HOME           : pins os.homedir() → tmpdir
//   - IRCODER_DISABLE_PROJECT_CONFIG : skip walking up for ircoder.json
//   - IRCODER_PURE                : skip external plugin discovery + install
//   - IRCODER_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `ircoder.run` is fully wired. The shape supports adding more
// builders (`ircoder.serve(opts)`, `ircoder.acp(opts)`, `ircoder.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of IrcoderCli.
...
const ircoderRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(ircoderRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    IRCODER_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    IRCODER_CONFIG_CONTENT: configJson,
    IRCODER_DISABLE_PROJECT_CONFIG: "1",
    IRCODER_PURE: "1",
    IRCODER_DISABLE_AUTOUPDATE: "1",
    IRCODER_DISABLE_AUTOCOMPACT: "1",
    IRCODER_DISABLE_MODELS_FETCH: "1",
...
// Typed equivalent of constructing argv for `ircoder run`. New flags should
...
// `ircoder serve` is a long-lived process — it never exits on its own.
...
// `ircoder acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type IrcoderCli = {
  // High-level: run a single prompt against the test model. Short-lived.
  readonly run: (message: string, opts?: RunOpts) => Effect.Effect<RunResult>
  // Spawn `ircoder serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `ircoder acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("ircoder.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("ircoder.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "ircoder server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `ircoder serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("ircoder.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const ircoder: IrcoderCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, ircoder })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `ircoder.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "ircoder") {
...
// every call site. Use this for any test that needs the ircoder CLI fixture.
//
...
// resources (e.g. `ircoder.serve`) without an extra `Effect.scoped` wrapper —
