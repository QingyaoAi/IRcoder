...
describe("reference", () => {
  it.live("resolves supported local and git config forms", () =>
    Effect.gen(function* () {
      const root = path.resolve("ircoder-reference-root")
    ...
    provideTmpdirInstance(
      (_dir) =>
        Effect.gen(function* () {
          ...
          const cache = path.join(Global.Path.repos, "github.com", "ircoder-reference-test", "repo")
          ...
          const remoteDir = path.join(remoteRoot, "ircoder-reference-test")
        ...
        }),
      {
        config: {
          reference: {
            docs: "ircoder-reference-test/repo",
          },
        },
      },
    ),
  ...
  scout.live("refreshes configured git references on new instance init", () =>
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const cache = path.join(Global.Path.repos, "github.com", "ircoder-reference-refresh", "repo")
      yield* fs.remove(cache, { recursive: true }).pipe(Effect.ignore)
      yield* Effect.addFinalizer(() => fs.remove(cache, { recursive: true }).pipe(Effect.ignore))

      const source = yield* tmpdirScoped({ git: true })
      const remoteRoot = yield* tmpdirScoped()
      const remoteDir = path.join(remoteRoot, "ircoder-reference-refresh")
      const remoteRepo = path.join(remoteDir, "repo.git")
      ...
      yield* githubBase(
        ...
        provideTmpdirInstance(
            ...
            }),
          {
            config: {
              reference: {
                docs: "ircoder-reference-refresh/repo",
              },
            },
          },
        ),
