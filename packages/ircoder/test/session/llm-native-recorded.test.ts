...
const RECORDED_SCENARIOS = [
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["ircoder", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["ircoder", "zen", "native", "tool-loop"],
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the ircoder LLM stack remain real.
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "ircoder.json"),
      JSON.stringify({ $schema: "https://ircoder.ai/config.json", ...scenario.config(model) }),
    ),
  )
