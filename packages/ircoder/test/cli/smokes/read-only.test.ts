...
describe("ircoder read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // IRCODER_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ ircoder }) =>
      Effect.gen(function* () {
        const r = yield* ircoder.spawn(["mcp", "list"])
        ircoder.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ ircoder }) =>
      Effect.gen(function* () {
        const r = yield* ircoder.spawn(["providers", "list"])
        ircoder.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ ircoder }) =>
      Effect.gen(function* () {
        const r = yield* ircoder.spawn(["models"])
        ircoder.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ ircoder }) =>
      Effect.gen(function* () {
        const r = yield* ircoder.spawn(["agent", "list"])
        ircoder.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh IRCODER_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ ircoder }) =>
      Effect.gen(function* () {
        const r = yield* ircoder.spawn(["session", "list"])
        ircoder.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ ircoder }) =>
      Effect.gen(function* () {
        const r = yield* ircoder.spawn(["stats"])
        ircoder.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under IRCODER_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ ircoder }) =>
      Effect.gen(function* () {
        const r = yield* ircoder.spawn(["db", "path"])
        ircoder.expectExit(r, 0, "db path")
