...
describe("ircoder acp-next (subprocess)", () => {
  cliIt.live(
    "responds to initialize behind IRCODER_ACP_NEXT",
    ({ ircoder }) =>
      Effect.gen(function* () {
        const acp = createAcpClient(yield* ircoder.acp({ env: { IRCODER_ACP_NEXT: "1" } }))
        ...
        expect(initialized.agentInfo?.name).toBe("IRcoder")
        expect(initialized.authMethods?.[0]?.id).toBe("ircoder-login")
  ...
  cliIt.live(
    ...
    ({ ircoder }) =>
      Effect.gen(function* () {
        ...
        expect(methodId).toBe("ircoder-login")
    ...
    ({ ircoder }) =>
      Effect.gen(function* () {
        const exitedPromise = yield* Effect.scoped(
          Effect.gen(function* () {
            const acp = yield* ircoder.acp({ env: { IRCODER_ACP_NEXT: "1" } })
            return acp.exited
          }),
        )
  ...
  cliIt.live(
    ...
    ({ ircoder }) =>
      Effect.gen(function* () {
        const acp = createAcpClient(yield* ircoder.acp())
        const initialized = expectOk(yield* acp.request<InitializeResponse>("initialize", { protocolVersion: 1 }))
