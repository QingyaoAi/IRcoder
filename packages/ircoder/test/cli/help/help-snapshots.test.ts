...
// terminal sizes. The default ircoder tui command is excluded —
// `ircoder --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `ircoder --help` prints today;
...
// top-level help on `--help` and exits 1; not a real ircoder command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("ircoder CLI help-text snapshots", () => {
    ...
    ({ ircoder }) =>
      Effect.gen(function* () {
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* ircoder.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`ircoder ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`ircoder ${argv.join(" ")} --help`)
        }
