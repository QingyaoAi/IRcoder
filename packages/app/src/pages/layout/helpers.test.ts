...
import { type Session } from "@ircoder/sdk/v2/client"
import {
  childSessionOnPath,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("ircoder://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("ircoder://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("ircoder://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("ircoder://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("ircoder://open-project")).toBeUndefined()
    expect(parseDeepLink("ircoder://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "ircoder://open-project?directory=/a",
      "ircoder://other?directory=/b",
      "ircoder://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("ircoder://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("ircoder://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("ircoder://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("ircoder://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "ircoder://new-session?directory=/a",
      "ircoder://open-project?directory=/b",
      "ircoder://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __IRCODER__: {
        deepLinks: ["ircoder://open-project?directory=/a"],
      },
    } as unknown as Window & { __IRCODER__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["ircoder://open-project?directory=/a"])
