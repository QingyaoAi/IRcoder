...
import { AISDK } from "@ircoder/core/aisdk"
import { ModelV2 } from "@ircoder/core/model"
import { PluginV2 } from "@ircoder/core/plugin"
import { DynamicProviderPlugin } from "@ircoder/core/plugin/provider/dynamic"
...
function tempEntrypoint(source: string) {
  return Effect.acquireRelease(
    Effect.promise(async () => {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "ircoder-provider-dynamic-"))
