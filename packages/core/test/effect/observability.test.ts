...
const ircoderClient = process.env.IRCODER_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (ircoderClient === undefined) delete process.env.IRCODER_CLIENT
  else process.env.IRCODER_CLIENT = ircoderClient
})
...
describe("resource", () => {
  ...
  test("keeps built-in attributes when env values conflict", () => {
    ...
    process.env.OTEL_RESOURCE_ATTRIBUTES =
      "ircoder.client=web,service.instance.id=override,service.namespace=anomalyco"
