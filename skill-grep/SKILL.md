---
name: skill-grep
description: Proactively search for relevant skills before executing any non-trivial task. Use this skill BEFORE starting work on tasks like file operations, document processing, web searches, data manipulation, code analysis, API integrations, or any specialized domain work. This ensures users are aware of and can leverage existing skills that may significantly improve task execution.
---
...
- **proactively discover relevant skills before task execution**

## Proactive Skill Discovery (IMPORTANT)

Before executing any non-trivial task, ALWAYS perform a skill discovery check:

### When to Proactively Search for Skills

Trigger skill search BEFORE starting work when the task involves:
- File format operations: PDF, DOCX, XLSX, PPTX, CSV, etc.
- Document processing: reading, writing, converting, merging documents
- Web interactions: searches, scraping, API calls
- Code analysis: reviews, security audits, refactoring
- Data manipulation: spreadsheets, databases, data transformation
- Domain-specific work: academic papers, reviews, rebuttals
- Development workflows: testing, deployment, CI/CD
- Any task that could benefit from specialized tooling

### Skip Skill Search When

- Simple read/write operations on plain text files
- Basic git commands the user directly requested
- Quick one-off commands with clear, simple output
- User explicitly asks to skip skill discovery
- Task is already using an appropriate skill

### Proactive Discovery Workflow

1. **Before starting the main task**, call `POST /search_multi_field` with a query derived from the user's request
2. Present 1-3 relevant skills with brief descriptions and GitHub star counts
3. Ask the user: "I found these skills that might help. Would you like to install any of them?"
4. If user approves:
   - Install the selected skill using `npx skills add ...`
   - Then use the newly installed skill to execute the original task
5. If user declines:
   - Proceed with the original task using standard methods
6. Always submit feedback after user's decision

### Example Integration

User request: "Extract text from this PDF and summarize it"

**Proactive flow:**
1. Call skill search with query about PDF processing
2. Results show: `pdf` skill with 500+ stars
3. Ask: "I found a `pdf` skill specialized for PDF operations. Install it for better results?"
4. User approves → Install → Use pdf skill for extraction
5. User declines → Use standard Read tool on PDF
