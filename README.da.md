...
# Pakkehåndteringer
npm i -g ircoder-ai@latest        # eller bun/pnpm/yarn
scoop install ircoder             # Windows
choco install ircoder             # Windows
brew install anomalyco/tap/ircoder # macOS og Linux (anbefalet, altid up to date)
brew install ircoder              # macOS og Linux (officiel brew formula, opdateres sjældnere)
sudo pacman -S ircoder            # Arch Linux (Stable)
paru -S ircoder-bin               # Arch Linux (Latest from AUR)
mise use -g ircoder               # alle OS
nix run nixpkgs#ircoder           # eller github:anomalyco/ircoder for nyeste dev-branch
```
...
IRcoder findes også som desktop-app. Download direkte fra [releases-siden](https://github.com/anomalyco/ircoder/releases) eller [ircoder.ai/download](https://ircoder.ai/download).
...
1. `$IRCODER_INSTALL_DIR` - Tilpasset installationsmappe
...
IRcoder har to indbyggede agents, som du kan skifte mellem med `Tab`-tasten.
...
Læs mere om [agents](https://ircoder.ai/docs/agents).

### Dokumentation

For mere info om konfiguration af IRcoder, [**se vores docs**](https://ircoder.ai/docs).

### Bidrag

Hvis du vil bidrage til IRcoder, så læs vores [contributing docs](./CONTRIBUTING.md) før du sender en pull request.

### Bygget på IRcoder

Hvis du arbejder på et projekt der er relateret til IRcoder og bruger "ircoder" som en del af navnet; f.eks. "ircoder-dashboard" eller "ircoder-mobile", så tilføj en note i din README, der tydeliggør at projektet ikke er bygget af IRcoder-teamet og ikke er tilknyttet os på nogen måde.

---

**Bliv en del af vores community** [Discord](https://discord.gg/ircoder) | [X.com](https://x.com/ircoder)
